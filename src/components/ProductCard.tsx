...
function AddToCartButton({ product }: { product: Product }) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`
        relative w-full overflow-hidden rounded-full px-5 py-2.5 text-sm font-medium
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden rounded-xl bg-zinc-100 dark:bg-zinc-800">
        ...
        {product.badge && (
          <span
            className={`
              absolute left-3 top-3 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-wider
              ${
                product.badge === "New"
        ...
        <div className="absolute inset-0 flex items-end p-3 opacity-0 transition-all duration-300 group-hover:opacity-100">
